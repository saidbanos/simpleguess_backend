...
exports.generateAsciiArt = async (req, res) => {
	try {
		...
		// First, fetch a specific item from the theme
		const item = await openaiService.fetchThemeItem(theme);
		//console.log(`Item selected: ${item}`); // Log the fetched item

		// Then, fetch the image URL using the fetched item
		const imageUrl = await openaiService.fetchImageUrl(item);
		console.log(`Item image URL: ${imageUrl}`); // Log the fetched image URL
